...
@section('page-title', 'Edit Dentist')
...
@section('content')
    ...
    <div class="bg-white rounded-xl border border-gray-200">
        <div class="p-6 border-b border-gray-100">
            <h2 class="text-lg font-bold text-gray-900">Edit Dentist</h2>
            <p class="text-gray-500 text-sm mt-1">Update the dentist's information</p>
        </div>

        <form action="{{ route('admin.dentists.update', $dentist->id) }}" method="POST" enctype="multipart/form-data" class="p-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div class="lg:col-span-2">
                    ...
                    <div class="flex items-center gap-6">
                        <div id="imagePreviewContainer" class="relative">
                            @if($dentist->image)
                                <img src="{{ asset('storage/' . $dentist->image) }}" class="w-32 h-32 rounded-xl object-cover border-2 border-gray-200">
                            @else
                ...
                <div>
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $dentist->name) }}" required class="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition">
                    @error('name')
                ...
                <div>
                    ...
                    <input type="email" name="email" id="email" value="{{ old('email', $dentist->email) }}" required class="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition">
                    @error('email')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="tel" name="phone" id="phone" value="{{ old('phone', $dentist->phone) }}" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition">
                    @error('phone')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="date" name="birthdate" id="birthdate" value="{{ old('birthdate', $dentist->birthdate?->format('Y-m-d')) }}" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition">
                    @error('birthdate')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="lg:col-span-2">
                    ...
                    <input type="text" name="address" id="address" value="{{ old('address', $dentist->address) }}" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition">
                    @error('address')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="lg:col-span-2">
                    <label class="flex items-center gap-3 cursor-pointer">
                        <input type="checkbox" name="is_active" value="1" {{ old('is_active', $dentist->is_active) ? 'checked' : '' }} class="w-5 h-5 rounded border-gray-300 text-blue-600 focus:ring-blue-500">
